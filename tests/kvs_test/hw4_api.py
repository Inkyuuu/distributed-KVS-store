...
class KvsClient:
    ...
    def _request(
        self, corr_id: int, node: _NodeLike, method: str, path: str, include_metadata: bool = True, **kwargs
    ) -> requests.Response:
    ...
    def send_view(self, node: _NodeLike, shard_view: Dict[str, Sequence[_NodeLike]]) -> bool:
        ...
        log(
            f"client {self.name} [{id}] -> {node.name}: view update with shards: "
            f"{ {k: [f'{n.name}(id={n.index})' for n in v] for k, v in shard_view.items()} }"
        )
